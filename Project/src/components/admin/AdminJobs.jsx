...
import { adminMockJobs } from './adminMockData';

export default function AdminJobs() {
    const [search, setSearch] = useState('');
    const [statusFilter, setStatusFilter] = useState('all');

    const filtered = adminMockJobs.filter(j => {
                    ...
                    <motion.div key={job.id} initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.04 }}
                        ...
                        <div className="flex items-start justify-between">
                            <div>
                                ...
                                <p className="text-white/40 text-sm mt-0.5">{job.company}</p>
                            </div>
                        ...
                        <div className="flex flex-wrap items-center gap-4 mt-4 text-xs text-white/40">
                            ...
                            <span className="flex items-center gap-1"><HiUsers className="w-3.5 h-3.5" /> {job.applicantCount} applicants</span>
                            <span className="flex items-center gap-1"><HiClock className="w-3.5 h-3.5" /> {job.deadline}</span>
                        </div>
                    </motion.div>
                ))}
            </div>
